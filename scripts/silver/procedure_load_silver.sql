...
Script Purpose:
  ...
  Actions Performed:
    ...
    - Inserts the transformed and cleansed data from the Bronze into the Silver Tables.
...
BEGIN
	...
	SET @batch_start_time = GETDATE();
		PRINT 'Loading Silver Layer Layer';
		PRINT '==============================================================';
		...
		CASE 
			...
			ELSE sls_price -- Recalculate price if original value is missing
		END AS sls_price
		...
		SELECT 
		CASE 
			 WHEN cid LIKE 'NAS%' THEN SUBSTRING(cid, 4, LEN(cid)) --Removed 'NAS' prefix if it was present
			 ELSE cid
		END AS cid,
		CASE 
